...
Looking back on how we achieved the F3 requirements, our team’s original plan from F2 turned out to be harder to follow than we expected. Our foundation from F2 wasn’t very organized, a lot of logic was packed into a few big files, most of which was mainly concentrated within "main.js". One of the main problems we faced was that the code became hard to read and maintain. With every new feature imeplementation the code only got larger and larger. To support the new features, we had to refactor parts of the project multiple times, each refactor risked breaking something that used to work, and also led to lots of scrapped reworks. For example, when we introduced the new input system and language handling, we had to change how the old interaction code was structured, and that sometimes made room transitions or basic interactions stop working the way we intended. Overall, this was a frustrating issue because fixing one problem often led to creating another.

Even though the refactoring was messy and sometimes discouraging, we got better at reading and understanding our own code, debugging weird bugs caused by interactions between systems, and communicating about changes so we didn’t step on each other’s work as much. By the end of F3, the project still isn’t perfectly organized, but it felt good to see how the game has evolved from a basic 3D scene and physics working in to building an multi room game with an inventory, a continuous oxygen system. In addiiton, the addiitonal features such mobile input, theme support, and multiple language support were all areas we haven't explored before.

Overall, as we worked on the F3 requirements, we realized and learned that starting with a rough foundation made everything more complicated and time consuming, especially when the game’s scope and requirements were undefined and unclear in the beginning. In that sense, the hardest parts of this project are also what gave us the most useful lessons about planning, scoping, and writing code that can handle growth and expansion without constantly breaking. If we were to do this again, we would spend more time early on organizing the code, separating code responsibilities into smaller modules, and thinking about how future features would fit in, instead of trying to reshape everything on the fly through out the project.
